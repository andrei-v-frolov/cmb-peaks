...
for i in n ksdev gamma sigma alpha local; do
	map2gif -xsz 800 -bar .true. -inp peaks-$i.fits -out "!peaks-$i.gif"
done

# and the thresholded GIFs
for i in ksdev local; do
	map2gif -xsz 800 -bar .true. -min 0.5137003373 -inp peaks-$i.fits -out "!peaks-$i-95.gif"
	map2gif -xsz 800 -bar .true. -min 0.7170542898 -inp peaks-$i.fits -out "!peaks-$i-99.gif"
done
